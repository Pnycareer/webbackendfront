...
const FaqsTable = () => {
  ...
  useEffect(() => {
    if (query) {
      const index = faqData.findIndex((course) =>
  ...
  const handleDelete = async (courseId, faqId) => {
  const confirmDelete = window.confirm("Are you sure you want to delete this FAQ?");
  if (!confirmDelete) return;

  try {
    const res = await axios.delete(`/courses/faqs/${courseId}/${faqId}`);
    ...
    if (res.data?.success) {
      ...
      setFaqData((prev) =>
        prev.map((course) =>
          course._id === courseId
            ? { ...course, faqs: course.faqs.filter((faq) => faq._id !== faqId) }
            : course
        )
      );
    } else {
  ...
  const filteredCourses = faqData.filter((course) =>
    course.courseName?.toLowerCase().includes(query.toLowerCase())
  );
  ...
  return (
    <div className="min-h-screen w-full bg-gray-100 flex justify-center items-start p-4 md:p-10">
      <div className="w-full max-w-6xl bg-white shadow-lg rounded-2xl p-6 md:p-10">
        <h2 className="text-2xl font-bold text-gray-800 mb-6 text-center">Course FAQs</h2>
                            ...
                            {course.faqs.map((faq) => (
                              ...
                              >
                                ...
                                <div className="mt-2 flex gap-4 text-xs">
                                  <Link
                                    to={`/dashboard/editfaqs/${course._id}/${faq._id}`}
                                    className="text-blue-600 hover:underline"
                                  >
                                    Edit
                                  </Link>
                                  <button
                                    onClick={() => handleDelete(course._id, faq._id)}
